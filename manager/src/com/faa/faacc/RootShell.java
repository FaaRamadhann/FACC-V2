 ...
 * Dipakai FAACC Manager untuk memanggil CLI module:
 * {@code su -c "faacc --scan --json"} dkk.
 */
public class RootShell {
    ...
    public static boolean hasRoot() {
        try {
            Result r = exec("echo faacc-root-ok", 15000);
            return r != null && r.code == 0 && r.out.contains("faacc-root-ok");
        } catch (Exception e) {
            return false;
        }
    }

    /** Cek apakah CLI module FAACC terpasang & bisa jalan. */
    public static boolean hasFacc() {
        try {
            Result r = exec("faacc --version", 20000);
            return r != null && r.code == 0 && r.out.contains("FAACC");
        } catch (Exception e) {
            return false;
        }
    }
